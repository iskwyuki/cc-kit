...
1. **スクリプトの場所を解決**する。この skill と同じ配信元に `wt-parallel/scripts/wt-new.sh` がある。
   最初に見つかったものを絶対パスに直して以降で使う:
   - `${CLAUDE_PLUGIN_ROOT}/skills/wt-parallel/scripts/wt-new.sh`（plugin として読まれている場合）
   - `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/wt-parallel/scripts/wt-new.sh`（user スコープに置いた端末）
   - `.claude/skills/wt-parallel/scripts/wt-new.sh`（project スコープに置いたリポジトリ）
