...
- PR 番号（例: `123`）→ 指定 PR
- レベル（`lite` / `standard` / `full`）→ そのレベルで回す。`full 123` のように PR 番号と併記できる

**レベルを省略した場合は `standard`（4 エージェント）。** 日常の PR はこれで足りる。
`full`（7 エージェント）は取りこぼしのコストが高い変更に限って明示する — 検問・認証・権限まわり、
広範な書き換え、外部に配布される asset など。`lite`（1 エージェント）は typo 修正や定型の差分向け。

> 既定を `full` にしていた時期があったが、通常の PR には重すぎた（1 周で 5 エージェント × 深掘り）。
> 深さが要るかは差分の性質で決まるので、**呼ぶ側が明示する**方式にした。
...
`/code-review <レベル> pr <番号>` を実行する（検証パス込み）。レベルは引数で受け取ったもの、
省略時は `standard`。**このレベルは 2 周目でも変えない**（後述 Step 5）。
...
**第 2 周は第 1 周と同じレベルで回す。** `/code-review` 自身の Step 7 には「Critical 自動修正後の
再レビューは Lite 固定」という規定があるが、あれは `/code-review` 内部のループの話であって、
このスキルの周回には適用しない。第 2 周を勝手に軽くすると、修正で新たに入った欠陥を
取りこぼしたまま「収束」と誤判定する。
